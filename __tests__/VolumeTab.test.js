...
describe('Volumes Tab', () => {
  ...
  it('renders each volume in state', () => {
    expect(wrapper.find('div.containers').children().length).toBe(
      props.volumeContainersList.length
    );
  });
  ...
  it('renders the correct number of containers for each volume', () => {
    expect(
      wrapper.find('.box').at(0).find('.volume-container-details').length
  ...
  it('each container renders correct properties from state', () => {
    expect(
      wrapper.find('.volume-container-details').at(0).childAt(1).text()
    ).toEqual(props.volumeContainersList[0].containers[0].Names);
    expect(wrapper.find('ul').at(0).childAt(0).text()).toEqual(
      `Status: ${props.volumeContainersList[0].containers[0].State}`
    );
    expect(wrapper.find('ul').at(0).childAt(1).text()).toEqual(
      `Running For: ${props.volumeContainersList[0].containers[0].Status}`
    );
  });
});
