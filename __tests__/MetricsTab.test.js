...
describe('Shallow rendered Metrics for chart', () => {
  ...
  it('Should render Pie chart data properly', () => {
    ...
    enzymeWrapper.find('p.legend-text').forEach((element) => {
      const value = element.text().split(' ');
      const percentage = parseInt(value[1].substr(0, value[1].length - 4));
  ...
  it('should render Net I/O data properly', () => {
    enzymeWrapper.find('p.chart-number').forEach((element, i) => {
      if (i === 0) {
        const value = element.text().split('/');
        const IOFront = parseInt(value[0].substr(0, value[0].length - 2));
        const IOBack = parseInt(value[1].substr(0, value[1].length - 2));
  ...
  it('should render Block I/O data properly', () => {
    enzymeWrapper.find('p.chart-number').forEach((element, i) => {
      if (i === 1) {
        const value = element.text().split('/');
        const IOFront = parseInt(value[0].substr(0, value[0].length - 1));
        const IOBack = parseInt(value[1].substr(0, value[1].length - 1));
