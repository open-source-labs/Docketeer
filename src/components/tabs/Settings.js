...
const Settings = (props) => {
    ...
    if (!mobileNumber) alert('Please enter phone number');
      ...
      if (isNaN(Number(mobileNumber)))
        ...
        query(
          ...
          (err, res) => {
            // ADDED 2 COMMAS AFTER MOBILENUMBER -> MAKE SURE THAT IT WORKS
            if (err) {
      ...
      query(
        ...
        (err, res) => {
          if (err) {
            ...
            console.log(`*** Inserted ${res} into users table. ***`);
            console.log(
              'global state before state update: ',
              props.notificationFrequency,
            ); // CHECK HOW THE STATE HAS CHANGED
            props.addNotificationFrequency(frequency); // ADDING TO GLOBAL STATE
            console.log(
              'global state after state update: ',
              props.notificationFrequency,
            ); // CHECK HOW THE STATE HAS CHANGED
          }
        },
  ...
  const renderAllContainersList = allContainersList.map((container, i) => {
    console.log('containers in settings', container);
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell align="center">
                  Memory exceeds attribute value
                </TableCell>
                <TableCell align="center">
                  CPU exceeds attribute value
                </TableCell>
